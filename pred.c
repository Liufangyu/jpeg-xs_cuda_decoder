...
int tco_pred_ver_compute_predictor(int gcli_top, int gtli_top, int gtli)
{
	return MAX(gcli_top, MAX(gtli, gtli_top));
}

int tco_pred_ver(gcli_data_t* gcli_buf, gcli_data_t* gcli_buf_top, int buf_len, gcli_pred_t* pred_buf,
	gcli_data_t* predictors, int gtli, int gtli_top)
{
	int i;
	if (buf_len > 0)
	{
		for (i = 0; i < buf_len; i++)
		{
			predictors[i] = tco_pred_ver_compute_predictor(gcli_buf_top[i], gtli_top, gtli);
...
int tco_pred_ver_inverse(gcli_pred_t* pred_buf, gcli_data_t* gcli_buf_top, int buf_len, gcli_data_t* gcli_buf, int gtli, int gtli_top)
{
	int i;
	int top;
	if (buf_len > 0)
	{
		for (i = 0; i < buf_len; i++)
		{
			top = tco_pred_ver_compute_predictor(gcli_buf_top[i], gtli_top, gtli);
