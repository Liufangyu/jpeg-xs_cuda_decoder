...
int yuv16_decode(char* filename, xs_image_t* im, int little_endian)
{
	int i,pix;
	FILE* f_in;
	uint16_t buf[4];
...
int yuv16_encode(char* filename, xs_image_t* im, int little_endian)
{
	int file_len;
	int i=0;
	FILE* f_out;
	...
	if (im->ncomps == 3)
	{
		...
		{
			fprintf(stderr, "YUV16 only supports 422 image data\n");
			return -1;
		}
	}
	else
	{
		fprintf(stderr, "unsupported image layout, cannot save as YUV16\n");
		return -1;
	}
	...
	file_len = im->width * im->height * sizeof(uint16_t) * 2;
	while (file_len > 0)
	{
